...
| `No_Of_Containers` | Total number of containers in this tracking |
| `Tracking_Status` | Current status of the tracking. Values: `Pending Departure`, `Departed from Origin`, `Arrived Destination`, `Delivered` |
...
| `Event Type` | Type or category of the event |
...
|-----------|-----------|---------|
| `{Stop} isActual = 1` AND `{Stop} Occurred = Yes` | `[A]` | Actual date; stop already reached |
| `{Stop} isActual = 1` AND `{Stop} Occurred = No` | `(A)` | Actual date; stop not yet reached |
...
| Condition | Indicator | Meaning |
|-----------|-----------|---------|
| `Event Date isActual = 1` AND `Event Occurred = Yes` | `[A]` | Actual date; event already occurred |
| `Event Date isActual = 1` AND `Event Occurred = No` | `(A)` | Actual date; event not yet occurred |
