import os, json, math, time, pyodbc
from collections import defaultdict
from geopy.geocoders import Nominatim

SCRIPT_DIR   = r'C:\Users\hangs\OneDrive\GitHub\Sealine-Database'
TRACK_NUMBER = '263641182'
CONTAINER    = 'CAAU5231059'

# ── Geocoding helpers ─────────────────────────────────────────────────────────
_geo   = Nominatim(user_agent="sealine-route-map/1.0")
_cache = {}
...
def _geocode(name):
    ...
    for q in candidates:
        ...
        try:
            time.sleep(1.1)
            r = _geo.geocode(q, timeout=10)
            if r:
                val = (r.latitude, r.longitude)
                break
        except Exception:
            continue
    _cache[name] = val
...
def build_coord_fix_map(rows):
    """
    Detect coordinate collisions: when 2+ different location names share the
    same (lat, lng), those coords are leaked/wrong. Geocode only those names.
    Unique DB coords are trusted as-is.
    """
    coord_to_names = defaultdict(set)
    for r in rows:
    ...
    for r in rows:
        ...
        if lat is None or lng is None or _haversine_km(lat, lng, geo[0], geo[1]) > 200.0:
            fix_map[loc] = geo
            print(f'  [GEOCODE FIX] "{loc}": DB ({lat:.3f},{lng:.3f}) -> ({geo[0]:.4f},{geo[1]:.4f})')
...
# Build coord correction map (collision detection only)
print('Detecting coordinate collisions...')
fix_map = build_coord_fix_map(rows)

# Build events with corrected coords, deduplicate
...
for r in rows:
    ...
    if key in seen:
        continue
    seen.add(key)
    dt_str = date.strftime('%Y-%m-%d %H:%M') if date else ''
...
print(f'\nEvents plotted: {len(containers[CONTAINER])}')
for e in containers[CONTAINER]:
    print(f'  Seq={e["seq"]:>3}  {e["loc"]:<40} ({e["lat"]:>9.4f},{e["lng"]:>10.4f})  {e["date"]}  {"Actual" if e["actual"] else "Est"}  {e["desc"]}')
...
legend.onAdd = () => {
  ...
  d.innerHTML = '<b style="color:#7eb8e6">Legend</b><br>' +
    ...
    '&#10230; Arrow = travel direction<br>' +
    '<span style="color:#cc8800">&#9711;</span> = Estimated date';
  return d;
};
legend.addTo(map);
</script>
...
filename = 'route_263641182_CAAU5231059.html'
