        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "Trigger: ANY request to show a city, port, or location on a map, including: "
                    ...
                    "Workflow: (1) run execute_sql to get lat/lon — "
                    "for named ports/cities use: SELECT TOP 1 Name, Lat, Lng, Country FROM Sealine_Locations "
                    "WHERE (LOCode = '<locode>' OR Name LIKE '%<name>%') AND DeletedDt IS NULL. "
                    "For multiple locations: SELECT DISTINCT Name, Lat, Lng, Country FROM Sealine_Locations "
                    "WHERE LOCode IN (...) AND DeletedDt IS NULL. "
                    "(2) IMMEDIATELY call show_location_map with the lat/lon from the SQL result. "
                    "CRITICAL: Once you have lat/lon values from execute_sql, you MUST pass them "
                    "directly to show_location_map — do NOT call generate_plot. "
                    "Single location example — 'Jawaharlal Nehru, IN (INNSA) in the map': "
                    "Step 1: SELECT TOP 1 Name, Lat, Lng FROM Sealine_Locations WHERE LOCode='INNSA' AND DeletedDt IS NULL. "
                    "Step 2: show_location_map(title='Jawaharlal Nehru (INNSA)', "
                    "locations=[{\"name\":\"Jawaharlal Nehru (INNSA)\",\"lat\":<Lat>,\"lon\":<Lng>}]). "
                    "Bubble example: show_location_map(title='Top POL Ports', "
