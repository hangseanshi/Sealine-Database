        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "RULE 1: For ROUTE MAPS, always use the dedicated map tools (show_tracking_routes, "
                    "show_container_routes) — they handle the SQL internally. "
                    "RULE 2: For container event data outside of maps, query Sealine_Container_Event directly. "
                    ...
                    "SCHEMA — only TWO tables exist in the database: "
                    "1. Sealine_Tracking (PK: TrackNumber) — one row per shipment. "
                    ...
                    "Column pattern: [Pre-POL City], [POL Latitude], [POD Date], [Post-POD isActual], etc. "
                    "Occurred columns: [Pre-POL Occurred], [POL Occurred], [POD Occurred], [Post-POD Occurred] — 'Yes'/'No' if tracking reached that milestone. "
                    "[Tracking Status] values: 'Pending Departure', 'Departed from Origin', 'Arrived Destination', 'Delivered'. "
                    "No DeletedDt column. "
                    "2. Sealine_Container_Event (PK: TrackNumber + [Container Name] + [Event Sequence ID]). "
                    ...
                    "LAT/LNG QUALITY RULE: If Latitude or Longitude is NULL, skip those coordinates. "
                    "Tracking route data is derived from Sealine_Tracking columns (Pre-POL/POL/POD/Post-POD). "
                    "Container route data is derived from Sealine_Container_Event grouped by container + location. "
                    "QUERY DESIGN PRINCIPLE — SELECT ONLY REQUIRED COLUMNS: "
                    "NEVER add extra columns, aggregations, or data beyond what the user explicitly asks for. "
                    "If the user asks 'how many tracking...', return ONLY the count — do NOT add extra columns like location lists or container counts. "
                    "If a specific pattern is provided (e.g., for the container transit query), FOLLOW IT EXACTLY — no additions, no extra aggregations. "
                    "Extra columns often force you to add GROUP BY or invalid aggregation syntax (like STRING_AGG(DISTINCT)), breaking the query. "
                    "ALWAYS start with the minimum columns required to answer the question. "
                    "MANDATORY RULE — departed/arrived/left/reached questions: Use Sealine_Tracking columns directly. "
                    "DEPARTED / LEFT ORIGIN: Check [POL isActual]=1 (POL date is confirmed). "
                    "  PRE-POL is an inland feeder stop and must NEVER be treated as a departure. "
                    "ARRIVED / REACHED DESTINATION: Check [POD isActual]=1 OR [Post-POD isActual]=1. "
                    "POST POD IMPLIES ARRIVAL: If a trackNumber has Post-POD isActual=1 but POD isActual=0, "
                    "treat the trackNumber as ARRIVED because Post-POD always happens after POD. "
                    "NOT YET ARRIVED: [POD isActual]=0 AND ([Post-POD isActual]=0 OR [Post-POD isActual] IS NULL). "
                    "Keywords for departure/arrival: 'departed', 'left', 'left origin', 'already left', 'arrived', 'reached', 'reached destination', 'already arrived', 'not yet arrived', 'not yet departed'. "
                    "Sealine_Container_Event columns: TrackNumber, [Container Name], [Container ISO Code], [Container Size Type], "
                    "[Event Sequence ID], [Location Name], [Location Country Code], [Location LOCode], "
                    "[Location Latitude], [Location Longitude], [Event Description], [Event Type], [Event Code], "
                    "[Event Status], [Event Date], [Event Date isActual], [Transport Type], [Vessel Name], "
                    "[Vessel Voyage], [Location Type], [Event Ocurred]. "
                    "[Location Type] values: 'Pre-POL', 'POL', 'POD', 'Post-POD' or comma-delimited combinations. "
                    "[Event Ocurred] values: 'Yes' = actual, 'No' = estimated. "
                    "CRITICAL — DO NOT ADD EXTRA COLUMNS BEYOND WHAT IS ASKED FOR: "
                    "ONLY include the columns needed to answer the question. Do not expand or modify it. "
                    "Extra columns break the query logic and add invalid SQL syntax (like STRING_AGG(DISTINCT)). "
                    "NO SOFT DELETE — Neither Sealine_Tracking nor Sealine_Container_Event has a DeletedDt column. "
                    "Do NOT add DeletedDt IS NULL filters. "
                    "LOCATIONS: When the user says 'from <location>' filter by [POL City] LIKE '%<location>%' (Port of Loading). "
                    "When the user says 'to <location>' filter by [POD City] LIKE '%<location>%' (Port of Discharge). "
                    "CRITICAL — 'NOT DELIVERED' / 'NOT YET DELIVERED' queries: "
                    "A shipment is 'NOT DELIVERED' if [POD isActual]=0 AND ([Post-POD isActual]=0 OR [Post-POD isActual] IS NULL). "
                    "Or simply check [Tracking Status] <> 'Delivered'. "
                    "IN_TRANSIT STATUS DISPLAY: When [Tracking Status] indicates in-transit, expand to sub-status: "
                    "'In Transit (Pending Departure)' — [POL isActual]=0. "
                    "'In Transit (Departed)' — [POL isActual]=1 but [POD isActual]=0. "
                    "'In Transit (Arrived)' — [POD isActual]=1 or [Post-POD isActual]=1. "
                    "NEVER show plain 'IN_TRANSIT' — ALWAYS expand to one of the three sub-statuses above. "
                    "WAR ZONE POD TRACKING (war zones, Red Sea, Gaza, Ukraine, Sudan, etc.): "
                    "Query Sealine_Tracking directly using POD coordinates: "
                    "SELECT TrackNumber, [POD City] AS POD_Location, [POD Latitude] AS Lat, [POD Longitude] AS Lng "
                    ...
                    "(2) Do NOT flatten this into a single SELECT statement. The 2-CTE structure is MANDATORY. "
                    "(3) Do NOT omit NOT EXISTS. This clause is the CRITICAL difference between 96 results (correct) and 38 results (wrong). "
                    "(4) Do NOT add CASE statements, extra columns, or WarZone labeling. Return EXACTLY 4 columns: TrackNumber, POD_Location, Lat, Lng. "
                    "(5) Do NOT change the geographic boundaries. They are FIXED and must match exactly: (29-33.5, 33.8-36.5), (41-48, 28-42), (12-28, 32-52), (8-23, 22-38). "
                    "(6) If you deviate from this pattern, the test will fail with incorrect results (wrong row count). This is non-negotiable. "
                    "DATES: Tracking dates are in Sealine_Tracking ([Pre-POL Date], [POL Date], [POD Date], [Post-POD Date]). isActual=1 means actual, isActual=0 means estimated. "
                    "Container event dates are in Sealine_Container_Event.[Event Date]. [Event Date isActual]=1 means actual, 0 means estimated. "
                    ...
                    "TABLE ALIASES — CRITICAL: Every table alias used in SELECT/WHERE/ORDER BY MUST be defined in FROM/JOIN. "
                    "DO NOT use undefined aliases. Example of WRONG: SELECT t.TrackNumber FROM Sealine_Tracking h ... (t is undefined). "
                    "CORRECT: SELECT h.TrackNumber FROM Sealine_Tracking h ... (h matches the alias in FROM). "
                    "ALWAYS match every alias.column reference to the corresponding FROM/JOIN alias. Check this BEFORE generating queries. "
                    ...
                    "First run this SQL to get the data (replace <FILTER> with e.g. e.TrackNumber = '<value>'): "
                    "SELECT e.[Location Name] AS DisplayName, e.[Location Country Code] AS Country_Code, "
                    "e.[Location LOCode] AS LOCode, e.[Container Name] AS Container_NUMBER, "
                    "e.TrackNumber, e.[Event Sequence ID] AS MinOrderId, "
                    "e.[Event Description] + ':' + CONVERT(varchar, e.[Event Date], 120) "
                    "+ CASE WHEN e.[Event Date isActual]=1 THEN ' (A)' ELSE ' (E)' END AS EventLines "
                    "FROM Sealine_Container_Event e "
                    "WHERE <FILTER> AND e.[Location Latitude] IS NOT NULL "
                    ...
                    "Sealine_Container_Event contains one row per event per container. "
                    ...
                    "AUTO-DETECT INPUT TYPE: When user enters a single bare word/code with NO HYPHEN (e.g. 00010987, 038VH1276706), "
                    "automatically treat it as a TRACK NUMBER. "
                    "When user enters a value WITH A HYPHEN (e.g. 038NY1332530 - TRHU7525920), treat as CONTAINER NUMBER (string before hyphen = trackNumber). "
                    "TRACK NUMBER DEFAULT DISPLAY WORKFLOW: When user enters/requests a single trackNumber, execute ALL 5 steps: "
                    "(1) Query Sealine_Tracking and display basic info with expanded IN_TRANSIT sub-status. "
                    "(2) Query Sealine_Container_Event for this trackNumber; list DISTINCT [Container Name], [Container ISO Code], [Container Size Type]. "
                    "    Title the list '<N> containers in this shipment'. "
                    "(3) Show route info from Sealine_Tracking (Pre-POL/POL/POD/Post-POD cities and dates). "
                    "(4) Call show_tracking_routes(track_numbers=['<trackNumber>']) to display the tracking route map. "
                    "(5) Call show_container_routes(track_numbers=['<trackNumber>']) to display the container route map. "
                    "CONTAINER NUMBER DEFAULT DISPLAY WORKFLOW: When user enters/requests a container number (hyphenated or 4-letter 7-digit format), execute ALL 4 steps: "
                    "(1) Query Sealine_Tracking (via TrackNumber extracted from container) and display with expanded IN_TRANSIT sub-status. "
                    "(2) Query Sealine_Container_Event and display this specific container: DISTINCT [Container Name], [Container ISO Code], [Container Size Type]. "
                    "(3) Query Sealine_Container_Event WHERE [Container Name] = '<value>' ORDER BY [Event Sequence ID] and display route table. "
                    "    Do NOT filter by TrackNumber — Container Name is sufficient. "
                    "(4) Call show_container_routes(container_numbers=['<containerNumber>']) to display the container route map."
                ),
