        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "Run this EXACT SQL (one row per unique location per container, all events as newline-separated text): "
                    "SELECT e.Container_NUMBER, e.TrackNumber, "
                    "TRY_CAST(COALESCE(f.Lat, l.Lat) AS FLOAT) AS Lat, "
                    "TRY_CAST(COALESCE(f.Lng, l.Lng) AS FLOAT) AS Lng, "
                    "COALESCE(f.Name, l.Name) AS LocationName, l.Country_Code, l.LOCode, "
                    "MIN(TRY_CAST(e.Order_id AS INT)) AS MinOrderId, "
                    "STRING_AGG(CONVERT(VARCHAR(10), e.Date, 120) + CASE WHEN e.Actual=1 THEN ' (A)' ELSE ' (E)' END + ': ' + e.Description, CHAR(10)) "
                    "WITHIN GROUP (ORDER BY TRY_CAST(e.Order_id AS INT)) AS EventLines "
                    ...
                    "GROUP BY e.Container_NUMBER, e.TrackNumber, COALESCE(f.Lat, l.Lat), COALESCE(f.Lng, l.Lng), "
                    "COALESCE(f.Name, l.Name), l.Country_Code, l.LOCode "
                    "ORDER BY e.TrackNumber, e.Container_NUMBER, MIN(TRY_CAST(e.Order_id AS INT)) ASC. "
                    "EventLines uses CHAR(10) (newline) as separator — the map renderer converts newlines to line breaks automatically. "
                    "Pass ALL rows from ALL tracking numbers in a SINGLE generate_plot call. "
                    "Sort rows by TrackNumber, then Container_NUMBER, then MinOrderId — no interleaving. "
                    "Build these parallel arrays from the sorted SQL rows (one entry per row): "
                    "lat=COALESCE(f.Lat,l.Lat), lon=COALESCE(f.Lng,l.Lng), "
                    "labels=Container_NUMBER+'<br>'+DisplayName+'/'+CountryCode+' ('+LOCode+')<br>'+EventLines, "
                    "groups=TrackNumber+'_'+Container_NUMBER (e.g. '038VH9480992_CAAU7870450'), "
                    "track_groups=TrackNumber (e.g. '038VH9480992'), "
                    "arrows=true. "
                    "Also pass tracking=TrackNumber when there is only ONE tracking number (e.g. tracking='038VH9480992'). "
                    ...
                    "CONTAINER STOP LABELS: label=Container_NUMBER+'<br>'+DisplayName+'/'+CountryCode+' ('+LOCode+')<br>'+EventLines. "
                    ...
                    "'<TrackNumber><br><LocationName/CountryCode (LOCode)><br><RouteType><br><Date YYYY-MM-DD> (A or E)<br><ContainerCount> container(s)'. "
                    "Use '(A)' when r.IsActual=1, '(E)' when r.IsActual=0. "
                    "Example: 'DALA71196300<br>Houston/US (USHOU)<br>Pol<br>2026-02-09 (A)<br>42 container(s)'. "
                    "EXACT example for 2 tracking numbers (AAA111: 2 stops, BBB222: 2 stops): "
                    "lat=[29.76,29.87,22.3,30.5], lon=[-95.36,121.54,114.17,32.2], "
                    "labels=[\"AAA111<br>Houston/US (USHOU)<br>Pol<br>2026-02-09 (A)<br>12 container(s)\","
                    "\"AAA111<br>Ningbo/CN (CNNGB)<br>Pod<br>2026-03-16 (E)<br>12 container(s)\","
                    "\"BBB222<br>Shenzhen/CN (CNSZX)<br>Pol<br>2026-02-10 (A)<br>5 container(s)\","
                    "\"BBB222<br>Port Said/EG (EGPSD)<br>Pod<br>2026-03-20 (E)<br>5 container(s)\"], "
                    ...
                    "SELECT COALESCE(f.Name, l.Name) AS DisplayName, l.Country_Code, l.LOCode, "
                    "e.Container_NUMBER, TRY_CAST(e.Order_id AS INT) AS OrderId, "
                    "CONVERT(VARCHAR(10), e.Date, 120) AS EventDate, "
                    "e.Actual, e.Description "
                    "FROM Sealine_Container_Event e "
                    "LEFT JOIN Sealine_Locations l ON e.TrackNumber = l.TrackNumber AND e.Location = l.Id "
                    "LEFT JOIN Sealine_Facilities f ON e.TrackNumber = f.TrackNumber AND e.Facility = f.Id "
                    "WHERE <FILTER> AND e.DeletedDt IS NULL AND l.DeletedDt IS NULL "
                    "ORDER BY COALESCE(f.Name, l.Name), e.Container_NUMBER, TRY_CAST(e.Order_id AS INT). "
                    ...
                    "Each container has container_number and events list ordered by OrderId ascending. "
                    "Each event has date (YYYY-MM-DD), actual (true if e.Actual=1, false if e.Actual=0), description. "
                    ...
                    "pass ALL rows from ALL tracking numbers in a SINGLE generate_plot call. "
                    "Sort rows: first by TrackNumber, then Container_NUMBER, then MinOrderId — NO interleaving between tracking numbers or containers. "
                    "Build these parallel arrays from the sorted SQL rows (one entry per row): "
                    "lat=l.Lat, lon=l.Lng, "
                    "labels=ContainerNumber+'<br>'+DisplayName+'/'+CountryCode+' ('+LOCode+')<br>'+EventLines, "
                    ...
                    "EventLines is the STRING_AGG result with CHAR(10) newline separator — pass it directly, the renderer handles indentation. "
                    "LocationName is COALESCE(f.Name, l.Name), CountryCode is l.Country_Code, LOCode is l.LOCode. "
