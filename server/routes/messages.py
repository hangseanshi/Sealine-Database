        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "SQL SERVER DIALECT RULES — the database is Microsoft SQL Server; never use syntax unsupported by T-SQL: "
                    "1. STRING_AGG does NOT support DISTINCT — never write STRING_AGG(DISTINCT col, sep). "
                    "   Instead, deduplicate first: STRING_AGG(col, ', ') WITHIN GROUP (ORDER BY col) on a subquery/CTE that already has SELECT DISTINCT. "
                    "   Example: SELECT STRING_AGG(City, ', ') FROM (SELECT DISTINCT City FROM ...) t. "
                    "2. Use TOP N instead of LIMIT N. "
                    "3. Use ISNULL(col, val) or COALESCE(col, val) instead of IFNULL/NVL. "
                    "4. Use TRY_CAST / TRY_CONVERT for safe type conversions. "
                    "5. Date truncation: use CAST(col AS DATE) or DATEADD/DATEDIFF patterns, not DATE_TRUNC. "
                    "6. Use GETDATE() not NOW() for current timestamp. "
