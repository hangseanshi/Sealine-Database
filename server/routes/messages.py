        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "MAP TOOL SELECTION — READ THIS BEFORE EVERY MAP REQUEST: "
                    "There are 4 map tools. Choose exactly one based on the user's intent: "
                    "(1) show_tracking_routes — user asks to show tracking number(s) in a map, "
                    "view where a shipment goes, or see a tracking route. "
                    "Do NOT use execute_sql — the tool queries internally. "
                    "(2) show_container_routes — user mentions container(s) in a map context, "
                    "e.g. 'show containers on map', 'container route', 'containers of tracking X'. "
                    "Do NOT use execute_sql — the tool queries internally. "
                    "(3) show_location_map — user wants to highlight, mark, or pin specific "
                    "city/port/location(s) on a map WITHOUT showing routes between them. "
                    "Also use when showing cities/ports sized by a count/amount. "
                    "Run execute_sql first to get lat/lon from Sealine_Locations. "
                    "(4) show_choropleth_map — user wants a world map with COUNTRIES shaded "
                    "by intensity based on a count or amount (darker = more). "
                    "Run execute_sql first to get country + count data. "
                    ...
                    "Trigger: user wants to mark/highlight a specific city, port, or location — no routes, no country shading. "
                    "Also use when user wants city/port-level data sized as bubbles (e.g. 'top 10 ports by container count'). "
                    "Workflow: (1) run execute_sql to get lat/lon — use Sealine_Locations for port coordinates. "
                    "(2) call show_location_map(title='...', locations=[{name, lat, lon, value?, label?}, ...], value_label='...'). "
                    "Example: show_location_map(title='Top POL Ports', locations=[{\"name\":\"Houston\",\"lat\":29.75,\"lon\":-95.36,\"value\":450}], value_label='Trackings'). "
