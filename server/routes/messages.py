        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "ABSOLUTE RULE — ENFORCED BEFORE ALL OTHERS: "
                    "The table Sealine_Container_Event DOES NOT EXIST. "
                    ...
                    "YES → you MUST query v_sealine_container_route and build a container map. "
                    ...
                    "or mentions containers with a tracking number, ONLY use the view v_sealine_container_route — no other table. "
                    "v_sealine_container_route links to Sealine_Header via TrackNumber. "
                    "One TrackNumber may have many rows in this view (one per unique location per container). "
                    "The unique identifier in this view is the combination of Container_NUMBER + MinOrderId. "
                    "Run this EXACT SQL (DO NOT change the table or add joins): "
                    ...
                    "WHERE <FILTER> AND v.Lat IS NOT NULL AND v.Lng IS NOT NULL "
                    "ORDER BY v.TrackNumber, v.Container_NUMBER, v.MinOrderId ASC. "
                    "EventLines is already aggregated in the view — pass it directly as-is. "
                    "The map renderer converts newlines (CHAR(10)) to line breaks automatically. "
                    "Pass ALL rows from ALL tracking numbers in a SINGLE generate_plot call. "
                    "LOOP ORDER for building arrays: "
                    "1. Process one TrackNumber at a time (outer loop). "
                    "2. Within each TrackNumber, process one Container_NUMBER at a time (inner loop). "
                    "3. Within each Container_NUMBER, rows are already sorted by MinOrderId ASC. "
                    "4. Draw an arrow line from each row to the next row within the SAME Container_NUMBER "
                    "   (i.e. connect lower MinOrderId → higher MinOrderId). "
                    "5. NEVER draw an arrow line across different Container_NUMBERs. "
                    ...
                    "(SELECT COUNT(DISTINCT cv.Container_NUMBER) FROM v_sealine_container_route cv "
                    "WHERE cv.TrackNumber = r.TrackNumber) AS ContainerCount "
