        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "When the user asks for shipments that are NOT DELIVERED or NOT YET DELIVERED, ALWAYS add NOT EXISTS to exclude shipments with actual POD arrival: "
                    ...
                    "This filters out ANY shipment where the POD or Post-POD has been ACTUALLY reached (IsActual=1). "
                    "DO NOT OMIT THIS — it is essential for 'not delivered' queries. Without it, you return all POD locations regardless of actual delivery status. "
                    "WAR ZONE POD TRACKING QUERIES (keywords: war zone, POD, not delivered, geographic, latitude, longitude, Red Sea, Persian Gulf, Mediterranean, Aden): "
                    "YOU MUST ALWAYS use this EXACT 2-CTE structure — NO EXCEPTIONS. The flat SELECT approach DOES NOT WORK for this query type. "
                    "Required structure (COPY EXACTLY): "
                    "WITH pod_locations AS ( "
                    "  SELECT DISTINCT h.TrackNumber, l.Name AS POD_Location, TRY_CAST(l.Lat AS FLOAT) AS Lat, TRY_CAST(l.Lng AS FLOAT) AS Lng "
                    ...
                    "  AND NOT EXISTS (SELECT 1 FROM Sealine_Route r2 WHERE r2.TrackNumber = h.TrackNumber AND r2.RouteType IN ('Pod', 'Post POD') AND r2.IsActual = 1 AND r2.DeletedDt IS NULL) "
                    "), "
                    "war_zone_pod AS ( "
                    "  SELECT TrackNumber, POD_Location, Lat, Lng FROM pod_locations "
                    "  WHERE (Lat BETWEEN 29 AND 33.5 AND Lng BETWEEN 33.8 AND 36.5) "
                    "     OR (Lat BETWEEN 41 AND 48 AND Lng BETWEEN 28 AND 42) "
                    "     OR (Lat BETWEEN 12 AND 28 AND Lng BETWEEN 32 AND 52) "
                    "     OR (Lat BETWEEN 8 AND 23 AND Lng BETWEEN 22 AND 38) "
                    ") "
                    "SELECT TrackNumber, POD_Location, Lat, Lng FROM war_zone_pod ORDER BY TrackNumber "
                    "CRITICAL RULES: "
                    "(1) NEVER use SELECT without CTEs for this pattern — ALWAYS use the 2-CTE structure above. "
                    "(2) First CTE (pod_locations) MUST include NOT EXISTS with (IsActual = 1) to exclude delivered shipments. "
                    "(3) Second CTE (war_zone_pod) MUST filter by geographic bounding boxes ONLY — do not duplicate filters. "
                    "(4) Final SELECT MUST return exactly 4 columns: TrackNumber, POD_Location, Lat, Lng. "
                    "(5) NEVER add Status, Country, LOCode, or Sealine_Code columns. "
                    "(6) Geographic boundaries ARE FIXED: (29-33.5, 33.8-36.5), (41-48, 28-42), (12-28, 32-52), (8-23, 22-38). "
