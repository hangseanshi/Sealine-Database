        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "(and does NOT mention containers), run a SINGLE SQL query: "
                    "SELECT l.TrackNumber, l.Name, l.Lat, l.Lng, l.Country, r.RouteType, r.Date, r.IsActual "
                    "FROM Sealine_Locations l "
                    "LEFT JOIN Sealine_Route r ON l.TrackNumber = r.TrackNumber AND l.Id = r.Location_Id "
                    "WHERE l.TrackNumber IN ('<val1>','<val2>',...) AND l.DeletedDt IS NULL "
                    "AND l.Lat IS NOT NULL AND l.Lng IS NOT NULL "
                    "ORDER BY l.TrackNumber, "
                    "CASE r.RouteType WHEN 'Pre-Pol' THEN 1 WHEN 'Pol' THEN 2 "
                    "WHEN 'Pod' THEN 3 WHEN 'Post-Pod' THEN 4 ELSE 5 END. "
                    ...
                    "labels=[\"POL - Houston<br>2026-02-09 (Actual)\",\"POD - Ningbo<br>2026-03-16 (Estimated)\","
                    "\"POL - Shenzhen<br>2026-02-10 (Actual)\",\"POD - Port Said<br>2026-03-20 (Estimated)\"], "
                    "groups=[\"AAA111\",\"AAA111\",\"BBB222\",\"BBB222\"], arrows=true. "
                    "Always include arrows=true. "
                    "STOP LABELS (tracking number): "
                    "'<ROUTETYPE> - <Location Name><br><Date YYYY-MM-DD> (<Actual/Estimated>)'. "
                    "Use 'Actual' when r.IsActual=1 else 'Estimated'. Example: 'POL - Houston<br>2026-02-09 (Actual)'. "
