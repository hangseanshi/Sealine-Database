        ...
        try:
            agent = SealineAgent(
                ...
                system_prompt=(
                    ...
                    "LAT/LNG QUALITY RULE: If Lat or Lng is blank, NULL, or cannot be converted to FLOAT via TRY_CAST, "
                    "ignore those coordinates entirely and use the LOCode field as the location reference instead. "
                    "Never plot or use a coordinate pair where either value fails TRY_CAST(x AS FLOAT). "
                    ...
                    "POST POD IMPLIES ARRIVAL: If a trackNumber has Post POD date with IsActual=1 but Pod date with IsActual=0, "
                    "treat the trackNumber as ARRIVED because Post POD always happens after POD. "
                    "If Post POD is actual, POD must be actual regardless of the IsActual flag on the Pod row. "
                    ...
                    "CRITICAL: This pattern SELECT clause MUST be EXACTLY: SELECT TrackNumber, COUNT(DISTINCT LocationName) AS DistinctTransitLocations "
                    "DO NOT add any other columns like STRING_AGG, Container counts, vessel names, locations list, or ANY other data. "
                    "Do NOT try to be 'helpful' by adding extra aggregations. The pattern requires EXACTLY 2 columns — no more, no less. "
                    ...
                    "When the pattern requires SELECT TrackNumber, COUNT(DISTINCT LocationName) ... — DO NOT add extra columns like STRING_AGG, Container counts, or other aggregations. "
                    "ONLY include the columns specified in the exact pattern. Do not expand or modify it. "
                    "Extra columns break the query logic and add invalid SQL syntax (like STRING_AGG(DISTINCT)). "
                    "Stick to the pattern EXACTLY as specified — no additions, no modifications, no extra aggregations. "
                    ...
                    "DEFINITION: A shipment is 'NOT DELIVERED' if it does NOT have any POD/Post POD route with IsActual=1 (actual arrival). "
                    "NOT based on Sealine_Header.Status field — Status is a summary field and WRONG for determining delivery status. "
                    "❌ WRONG: WHERE h.Status <> 'DELIVERED' AND h.Status <> 'COMPLETED' (This returns INCORRECT results) "
                    "✅ CORRECT: Use NOT EXISTS to check if ANY actual POD arrival exists: "
                    ...
                    "This is the ONLY correct way to determine 'not delivered'. "
                    "The IsActual = 1 flag is the definitive proof of actual delivery. Status field cannot be used as a proxy. "
                    "DO NOT OMIT THIS — without it, you return WRONG results (too many rows, incorrect shipments). "
                    "CRITICAL RULE: NEVER use Sealine_Header.Status to filter 'not delivered' queries. ALWAYS use NOT EXISTS with IsActual flag. "
                    "IN_TRANSIT STATUS DISPLAY: When Status = 'IN_TRANSIT', determine and display the expanded sub-status: "
                    "'In Transit (Pending Departure)' — if no POL row has EventLines NOT LIKE '%(E)%' (not yet departed from origin). "
                    "'In Transit (Departed)' — if departed from origin (POL actual) but no POD/Post-POD row has EventLines NOT LIKE '%(E)%' (not yet arrived). "
                    "'In Transit (Arrived)' — if arrived at destination (any POD or Post-POD row has EventLines NOT LIKE '%(E)%'). "
                    "NEVER show plain 'IN_TRANSIT' to the user — ALWAYS expand to one of the three sub-statuses above. "
                    "WAR ZONE POD TRACKING (when user mentions: war zones, war-affected, Red Sea, Persian Gulf, Mediterranean, Black Sea, Aden, Gaza, Ukraine, Sudan, POD locations, not delivered, in transit + geographic filter): "
                    "⚠️  **CRITICAL — THIS IS NON-NEGOTIABLE** — If you deviate from this structure, the query WILL FAIL and return WRONG RESULTS (42,000+ rows instead of 96). "
                    "THIS QUERY TYPE HAS A SPECIFIC MANDATORY STRUCTURE. DO NOT DEVIATE. DO NOT GENERATE FLAT SELECTS. DO NOT CHANGE THE LOGIC. "
                    "You MUST generate this EXACT structure — copy-paste from below if needed. If you don't know any values (coordinates, column names), copy the structure exactly AS-IS: "
                    ...
                    "ENFORCEMENT RULES: "
                    "(1) Do NOT use v_sealine_tracking_route or v_sealine_container_route views for this query. Use Sealine_Header, Sealine_Route, Sealine_Locations base tables ONLY. "
                    ...
                    "DO NOT use undefined aliases. Example of WRONG: SELECT t.TrackNumber FROM Sealine_Header h ... (t is undefined). "
                    "CORRECT: SELECT h.TrackNumber FROM Sealine_Header h ... (h matches the alias in FROM). "
                    ...
                    "(1) Query Sealine_Header and display basic info with expanded IN_TRANSIT sub-status. "
                    "(2) Query Sealine_Container for this trackNumber; list Container_NUMBER, Size_Type, Iso_Code. "
                    "    Title the list '<N> containers in this shipment'. "
                    "(3) Query v_sealine_tracking_route ORDER BY MinOrderId and display full route table. "
                    ...
                    "(1) Query Sealine_Header (via TrackNumber extracted from container) and display with expanded IN_TRANSIT sub-status. "
                    "(2) Query Sealine_Container and display this specific container: Container_NUMBER, Size_Type, Iso_Code. "
                    "(3) Query v_sealine_container_route WHERE container_NUMBER = '<value>' ORDER BY MinOrderId and display route table. "
                    "    Do NOT filter by TrackNumber — container_NUMBER is sufficient. "
